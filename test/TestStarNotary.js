...
// modify the state of the contract (tokenIdToStarInfo array, calling ERC721 _mint)
...
// modify the state of the contract (starsForSale array)
...
//     await instance.allowBuying(starId, user2, {from: user1});
...
it('lets user1 get the funds after the sale', async() => {
    ...
    await instance.createStar('awesome star', starId, {from: user1});
    const receipt = await instance.putStarUpForSale(starId, starPrice, {from: user1});
    ...
    console.log('before2: ' + balanceOfUser2BeforeTransaction);
    console.log('actual2: ' + balanceOfUser2AfterTransaction)
...
// it('lets user1 get the funds after the selling', async() => {
//     let instance = await StarNotary.deployed();
//     let user1 = accounts[1];
//     let user2 = accounts[2];
//     let starId = 6;
...
//     let balanceOfUser1BeforeTransaction = await web3.eth.getBalance(user1);
//     let balanceOfUser2BeforeTransaction = await web3.eth.getBalance(user2);
//     await instance.sellStar(starId, user2, {from: user1});
//     let balanceOfUser1AfterTransaction = await web3.eth.getBalance(user1);
//     let value1 = Number(balanceOfUser1BeforeTransaction) + Number(starPrice);
//     let value2 = Number(balanceOfUser1AfterTransaction);
//     assert.equal(value1, value2);
